...
class Trainer:
    def __init__(
        ...
        dataset = Dataset(
            tokenizer, self.config.context_size, data_dir, self.config.sample_every
        )
    ...
    def compute_loss(self, logits, y):
        return F.cross_entropy(logits.view(-1, self.config.vocab_size), y.view(-1))
...
def main(
    ...
    trainer = Trainer(
        project, tokenizer, checkpoint_dir, data_dir, output_dir, config, init_step
    )
    trainer.train()
